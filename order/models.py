...
class Order(CommonModel):
    ...
    status = models.SmallIntegerField('订单状态', choices=OrderStatus.choices, default=OrderStatus.SUBMIT)
    types = models.SmallIntegerField('订单类型', choices=OrderTypes.choices, default=OrderTypes.SIGHT_TICKET)
...
class OrderItem(CommonModel):
    ...
    flash_img = models.ImageField('商品主图')
    ...
    status = models.SmallIntegerField('状态', choices=OrderStatus.choices, default=OrderStatus.SUBMIT)
    remark = models.CharField('备注', max_length=255, null=True, blank=True)
...
class Payment(CommonModel):
    ...
    meta = models.CharField('其它数据', max_length=128, null=True, blank=True)
